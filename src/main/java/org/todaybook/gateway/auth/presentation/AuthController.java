 ...
 * </ul>
 *
 * <p>Refresh Token은 보안을 위해 HttpOnly Cookie로만 관리하며,
 * 클라이언트(JavaScript)에서는 직접 접근할 수 없습니다.
 *
...
public class AuthController {
   ...
   *
   * <p>Cookie의 maxAge와 서버의 Refresh Token TTL을
   * 동일한 기준으로 유지하기 위해 사용됩니다.
   */
   ...
   * </ul>
   *
   * <p>Refresh Token은 클라이언트에서 직접 접근하지 않으며,
   * 이후 토큰 재발급 시 자동으로 전송됩니다.
   */
  @PostMapping("/login")
   ...
   * 로그아웃 API.
   *
   * <p>HttpOnly Cookie에 저장된 Refresh Token을 기준으로
   * 서버(Redis)에 저장된 토큰을 무효화합니다.
   *
   * <p>로그아웃 이후에는 Refresh Token 쿠키를 즉시 삭제하여,
   * 이후 재발급 요청이 불가능하도록 처리합니다.
   */
  @PostMapping("/logout")
  public Mono<Void> logout(
    ...
    return authService.logout(refreshToken)
        .doOnSuccess(v -> deleteRefreshTokenCookie(response));
  }

  /**
   * Access Token 재발급 API.
   *
   * <p>HttpOnly Cookie로 전달된 Refresh Token을 검증한 뒤,
   * 새로운 Access Token과 Refresh Token을 발급합니다.
   *
   * <p>보안을 위해 Refresh Token은 회전(Rotate) 방식으로 재발급되며,
   ...
   * </ul>
   *
   * <p>Cookie의 만료 시간은 서버의 Refresh Token TTL과
   * 반드시 동일하게 유지되어야 합니다.
   */
